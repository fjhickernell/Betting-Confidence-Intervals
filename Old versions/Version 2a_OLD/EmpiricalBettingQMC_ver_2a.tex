...
in their sample size \cite{LEcEtal24a}. 
...
$$
K_t^+(m) = \prod_{i=1}^t(1+\lambda_i^+(m)(Y_i-m)\quad\text{and}\quad
K_t^-(m) = \prod_{i=1}^t(1-\lambda_i^+(m)(Y_i-m)
$$
...
As noted above, when $\theta = 1$, the best $n$ is $n=1$. In that case RQMC is the same as MC, using one uniformly distributed point in $[0,1]$. This result holds for any sampler that gets the ordinary MC rate, even if it has a favorable constant.  In particular, any MC variance reduction techniques that do not change the convergence rate will not lead to using $n>1$.  They will improve the asymptotic confidence interval width by reducing $\sigma_0$.  
...
the HBCI, both using RQMC, for 95\% confidence. 
The simulations presented here were run at: \url{https://github.com/aaditj1962161/Betting-Paper-Simulations-for-QMC/blob/paper-simulations/simulations/Fixed-time/Betting%20IID%20vs%20QMC.ipynb} and the data from the simulations are at \url{https://github.com/aaditj1962161/Betting-Paper-Simulations-for-QMC/blob/paper-simulations/simulations/Fixed-time/qmc_combined_results.csv}. The software we used comes from \url{https://github.com/WannabeSmith/betting-paper-simulations} and it calls on the HBCI and EBCI algorithms from \url{https://github.com/gostevehoward/confseq}.

We begin with finite $N$ and for some integrands with known RQMC variance, we find the $n$ that
...
For our HBCI computations, we used $\theta=1/2$. The software we used comes from \cite{hedged_algo} which has code to reproduce the figures in \cite{WauRam24a}. It calls the HBCI and predictable plug-in EBCI algorithms from \cite{bet_algo}.

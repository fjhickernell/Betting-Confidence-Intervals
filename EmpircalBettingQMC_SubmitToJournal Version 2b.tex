...
% ======================================================================
% SUBMISSION FILE MANIFEST (verified with LaTeX recorder .fls)
% ----------------------------------------------------------------------
% Main manuscript:
%   - EmpircalBettingQMC_SubmitToJournal Version 2b.tex
%
% Included TeX files:
%   - FJHDef.tex
%
...
%   - figwidthstoeb.pdf
%
% Bibliography (BibTeX inputs — used by bibtex, not listed in .fls):
%   - FJH25copy.bib
%   - FJHown25copy.bib
%   - ebci4rqmc.bib
%
% Build artifacts (usually not required by journals, but may be included):
%   - EmpircalBettingQMC_SubmitToJournal Version 2b.bbl  (final bibliography)
%   - EmpircalBettingQMC_SubmitToJournal Version 2b.aux
%   - EmpircalBettingQMC_SubmitToJournal Version 2b.out
%
% Notes:
%   - File list for TeX inputs/figures verified via:
%       latexmk -pdf -recorder "EmpircalBettingQMC_SubmitToJournal Version 2b.tex"
% ======================================================================
...
in their sample size \cite{naka:tuff:2024}. %more fitting than \cite{LEcEtal24a} which we had.
...
integral.  Ordinary quasi-Monte Carlo (QMC) 
...
Because plain QMC methods are deterministic, they lose the
...
The empirically best values of $n$ for HBCI are no larger
...
\end{align}
A confidence interval is strict when the sides of~\eqref{eq:ci} are equal.
...
for fixed $n$. That convergence is generally quicker for more nearly
...
% I just now got and fixed the galleys in that article so the following verbatim quote makes sense
\begin{quote}They consider a set $\cf$ of distributions on $\real$.
Letting $\mu(F)=\e(Y)$ for $Y\sim F\in\cf$, their conditions are:
...
$\lambda_i = \lambda_i(m)\in(-1/(1-m),1/m)$. 
...
$$
K_t^+(m) = \prod_{i=1}^t\bigl(1+\lambda_i^+(m)(Y_i-m)\bigr)\quad\text{and}\quad
K_t^-(m) = \prod_{i=1}^t\bigl(1-\lambda_i^+(m)(Y_i-m)\bigr)
...
As noted above, when $\theta = 1$, the best $n$ is $n=1$. In that case RQMC is the same as MC, using one uniformly distributed point in $[0,1]^d$. This result holds for any sampler that gets the ordinary MC rate, even if it has a favorable constant.  In particular, any MC variance reduction techniques that do not change the convergence rate will not lead to using $n>1$.  They will improve the asymptotic confidence interval width by reducing $\sigma_0$.  
...
the HBCI, both using RQMC, for 95\% confidence. We begin with finite $N$ and for some integrands with known RQMC variance, we find the $n$ that
...
For our HBCI computations, we used $\theta=1/2$. The simulations presented here were run at
\href{https://github.com/aaditj1962161/Betting-Paper-Simulations-for-QMC}{Betting-Paper-Simulations-for-QMC}. The software we used comes from \cite{hedged_algo} which has code to reproduce the figures in \cite{WauRam24a}. It calls the HBCI and predictable plug-in EBCI algorithms from \cite{bet_algo}.
...
scaled widths fluctuate % was fluctuation
...
discussions. We thank an anonymous reviewer for helpful comments.

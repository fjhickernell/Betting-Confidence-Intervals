...
%DIF DEL Version 1/EmpircalBettingQMC_SubmitToJournal V1 with edits.tex   Mon Dec 29 17:09:55 2025
%DIF ADD EmpircalBettingQMC_SubmitToJournal Version 2b.tex                Mon Dec 29 20:32:10 2025
...
in their sample size \DIFdelbegin %DIFDELCMD < \art{\cite{naka:tuff:2024}}%%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{naka:tuff:2024}}\hskip0pt%DIFAUXCMD
}\DIFaddend . %more fitting than \cite{LEcEtal24a} which we had.
...
integrals. When the dimension is high enough\DIFaddbegin \DIFadd{, }\DIFaddend then classical tensor product integration rules 
...
integral.  \DIFdelbegin \DIFdel{Quasi-Monte }\DIFdelend \DIFaddbegin \DIFadd{Ordinary quasi-Monte }\DIFaddend Carlo (QMC) 
...
\DIFdelbegin \DIFdel{Plain }\DIFdelend \DIFaddbegin \DIFadd{Because plain }\DIFaddend QMC methods are deterministic\DIFdelbegin \DIFdel{and so }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend they lose the
...
the best values of $n$ to use \DIFdelbegin \DIFdel{grew }\DIFdelend \DIFaddbegin \DIFadd{grow }\DIFaddend slowly with $N$.
The empirically best values of $n$ for HBCI \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend no larger
...
estimate $\mu$ by
$$
\hat\mu = \frac{1}n\sum_{i=1}^nf(\bsx_i).
$$
...
$$s^2 =\frac{1}{n-1}\sum_{i=1}^n(f(\bsx_i)-\hat\mu)^2$$
...
\,\biggl| \frac{1}n\sum_{i=1}^n1\{\bsx_i\in[\bszero,\bsa)\}
...
for
$$
\hat\mu = \frac{1}R\sum_{i=1}^R\hat\mu_i
\quad\text{and}\quad
S^2 = \frac{1}{R-1}\sum_{i=1}^R(\hat\mu_i-\hat\mu)^2.
$$
...
for fixed $n$. \DIFdelbegin %DIFDELCMD < \art{That convergence is generally quicker for more nearly
%DIFDELCMD < Gaussian $\hat\mu_i$.} %%%
\DIFdelend \DIFaddbegin \DIFadd{That convergence is generally quicker for more nearly
Gaussian $\hat\mu_i$. }\DIFaddend % We could quote Berry-Esseen but that brings in a third moment.  I think this remark should be enough.
...
Letting \DIFdelbegin %DIFDELCMD < \art{$\mu(F)=\e(Y)$ for $Y\sim F$}%%%
\DIFdelend \DIFaddbegin \DIFadd{$\mu(F)=\e(Y)$ for $Y\sim F$}\DIFaddend , their conditions are:
...
is that
$$
\e(Y) - \frac{1}R\sum_{i=1}^RY_i \le \sqrt{\frac{2\sigma^2\log(1/\delta)}R}
+ \frac{\log(1/\delta)}{3R}.
  $$
...
\DIFdelbegin \DIFdel{$\art{\lambda_i = \lambda_i(m)}\in(-1/(1-m),1/m)$}\DIFdelend \DIFaddbegin \DIFadd{$\lambda_i = \lambda_i(m)\in(-1/(1-m),1/m)$}\DIFaddend . 
...
$$
K_t^+(m) = \prod_{i=1}^t\bigl(1+\lambda_i^+(m)(Y_i-m)\DIFdelbegin %DIFDELCMD < \art{\bigr)}%%%
\DIFdelend \DIFaddbegin \bigr)\DIFaddend \quad\text{and}\quad
K_t^-(m) = \prod_{i=1}^t\bigl(1-\lambda_i^+(m)(Y_i-m)\DIFdelbegin %DIFDELCMD < \art{\bigr)}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bigr)
\DIFaddend % ABO: we were missing right parens.  I make the outer ones a touch bigger too.
$$
...
We take 
$$Y_i=\frac{1}n\sum_{k=1}^nf(\bsx_{i,k})$$
...
\begin{align*}
n^{-(\theta+1)/2} = 
\frac{2\log(2/\alpha)/(3N)}{(\theta-1)\sigma_0
\sqrt{2\log(2/\alpha)/N}}=
\frac{\sqrt{2\log(2/\alpha)}}{3(\theta-1)\sigma_0
\sqrt{N}}
\end{align*}
...
As noted above, when $\theta = 1$, the best $n$ is $n=1$. In that case RQMC is the same as MC, using one uniformly distributed point in \DIFdelbegin %DIFDELCMD < \art{$[0,1]^d$}%%%
\DIFdelend \DIFaddbegin \DIFadd{$[0,1]^d$}\DIFaddend . This result holds for any sampler that gets the ordinary MC rate, even if it has a favorable constant.  In particular, any MC variance reduction techniques that do not change the convergence rate will not lead to using $n>1$.  They will improve the asymptotic confidence interval width by reducing $\sigma_0$.  
...
\frac{H_\rqmc}{H_\mc}
&=
\frac{n^{(1-\theta)/2}\sqrt{2\sigma^2_0\log(2/\alpha)/N}+n\log(2/\alpha)/(3N)}{\sqrt{{2\sigma^2_0\log(2/\alpha)}/N}+\log(2/\alpha)/(3N)}\\
&=
\frac{n^{(1-\theta)/2}\sqrt{2\sigma^2_0N_\alpha}+n/3}{\sqrt{{2\sigma^2_0N_\alpha}}+1/3}.
\end{align*}
...
+(3(\theta-1)\sigma_0(N_\alpha/2)^{1/2})^{2/(\theta+1)}/3}{\sqrt{{2\sigma^2_0N_\alpha}}+1/3}\\
&=\frac{({3(\theta-1)})^{(1-\theta)/(\theta+1)}2^{\theta/(\theta+1)}
+(3(\theta-1))^{2/(\theta+1)}2^{-1/(\theta+1)}/3}{\sqrt{{2\sigma^2_0N_\alpha}}+1/3}(N_\alpha\sigma_0^2)^{1/(\theta+1)}\\
...
the HBCI, both using RQMC, for 95\% confidence. \DIFdelbegin %DIFDELCMD < \aadit{We begin with finite $N$ and for some integrands with known RQMC variance, we find the $n$ that
%DIFDELCMD < minimizes the interval widths from Bennett's inequality.
%DIFDELCMD < They are then the exact best $n$ for Bennett's inequality which the
%DIFDELCMD < HBCIs and EBCIs of \cite{WauRam24a} match asymptotically.
%DIFDELCMD < After that we present finite sample results on the
%DIFDELCMD < EBCIs and HBCIs for a variety of integrands differing in smoothness and dimension. The HBCIs are usually narrower.
%DIFDELCMD < Finally, we compare the empirically optimal $n$ for the HBCIs to the values that optimize Bennett's inequality. The HCBI widths approach those from Bennett's inequality for large $R$ and we find that the optimal $n$ for HBCI tend to be the same or smaller than the ones we get from Bennett's inequality.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We begin with finite $N$ and for some integrands with known RQMC variance, we find the $n$ that
minimizes the interval widths from Bennett's inequality.
They are then the exact best $n$ for Bennett's inequality which the
HBCIs and EBCIs of \mbox{%DIFAUXCMD
\cite{WauRam24a} }\hskip0pt%DIFAUXCMD
match asymptotically.
...
\DIFdelbegin %DIFDELCMD < \aadit{
%DIFDELCMD < \sout{The simulations presented here were run at:}
%DIFDELCMD < \url{https://github.com/aaditj1962161/Betting-Paper-Simulations-for-QMC/blob/paper-simulations/simulations/Fixed-time/Betting\%20IID\%20vs\%20QMC.ipynb}
%DIFDELCMD < \sout{and the data from the simulations are at}
%DIFDELCMD < \url{https://github.com/aaditj1962161/Betting-Paper-Simulations-for-QMC/blob/paper-simulations/simulations/Fixed-time/qmc_combined_results.csv}.
%DIFDELCMD < \sout{The software we used comes from}
%DIFDELCMD < \url{https://github.com/WannabeSmith/betting-paper-simulations}
%DIFDELCMD < \sout{and it calls on the HBCI and EBCI algorithms from}
%DIFDELCMD < \url{https://github.com/gostevehoward/confseq}.
%DIFDELCMD < 

%DIFDELCMD < \sout{We begin with finite $N$ and for some integrands with known RQMC variance, we find the $n$ that
%DIFDELCMD < minimizes the interval widths from Bennett's inequality.
%DIFDELCMD < They are then the exact best $n$ for Bennett's inequality which the
%DIFDELCMD < HBCIs and EBCIs of \cite{WauRam24a} match asymptotically.
%DIFDELCMD < After that we present finite sample results on the
...
%DIFDELCMD < %%%
\DIFdelend For our HBCI computations, we used $\theta=1/2$. \DIFdelbegin %DIFDELCMD < \aadit{The simulations presented here were run at
%DIFDELCMD < \url{https://github.com/aaditj1962161/Betting-Paper-Simulations-for-QMC}.}%%%
\DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{The simulations presented here were run at
}\url{https://github.com/aaditj1962161/Betting-Paper-Simulations-for-QMC.}\DIFadd{The }\DIFaddend software we used comes from \cite{hedged_algo} which has code to reproduce the figures in \cite{WauRam24a}. It calls the HBCI and predictable plug-in EBCI algorithms from \cite{bet_algo}.
...
scaled widths \DIFdelbegin %DIFDELCMD < \art{fluctuate} %%%
\DIFdelend \DIFaddbegin \DIFadd{fluctuate }\DIFaddend % was fluctuation
...
\var(\hat\mu) =\frac{1}{n^2}\sum_{\ell=1}^n\var( f(x_\ell))
\end{align}
...
$$
\var(\hat\mu) \asymp
\frac{1}{12n^3}\int_0^1f'(x)^2\rd x = \frac{5-e^{-2}}{48n^3}
$$
...
for our $\sigma^2_n$ and $R=N/n$\DIFdelbegin \DIFdel{double it
}\DIFdelend \DIFaddbegin \DIFadd{, double it,
}\DIFaddend and then divide the average HBCI width of 20 independent trials
